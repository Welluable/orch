 ...
 * - Columns: `SLUG ROLE STATE PHASE AGENT STARTED DURATION PID` — `ROLE` is
 *   what phase 4 adds; `STARTED` / `DURATION` predate it and stay, so the
 *   column-less sample in the phase-4 spec is not authoritative here.
...
describe('formatJobsTable — job tree rendering', () => {
  it('renders a coordinator with mixed children indented under it, ROLE column, and ordinary top-level jobs unindented', () => {
    ...
    // Top-level most-recent-first: solo before coordinator. STARTED / DURATION
    // are relative to now, so match them loosely and pin PID at end of row.
    assert.match(body, /^solo-meadow-a1b2\s+-\s+running\s+test-loop\s+claude\s+.+\s+13002$/m);
    assert.match(body, /^wise-pine-e904\s+coordinator\s+running\s+schedule\s+cursor\s+.+\s+12001$/m);
    ...
    assert.match(lines[parentIdx + 2], /^ {2}merry-elk-r4b1\s+worker\s+paused\s+code-loop\s+cursor\s+.+\s+12014$/);
    assert.match(lines[parentIdx + 3], /^ {2}tidy-heron-m2p9\s+integrate\s+pending\s+-\s+cursor\s+.+\s+-$/);

    // A finished child measures startedAt → finishedAt (4m 59s), not wall clock.
    assert.match(lines[parentIdx + 1], /\s4m 59s\s+-$/);
...
describe('orch pause|resume|stop — cascade for coordinator, leaf otherwise', () => {
  ...
  it('orch list shows the indented tree for a seeded coordinator + children', async () => {
    ...
    assert.equal(code, 0, stderr);
    assert.match(stdout, /^SLUG\s+ROLE\s+STATE\s+PHASE\s+AGENT\s+STARTED\s+DURATION\s+PID$/m);
