...
/**
 * Document-level `seq.json` `state` values.
 * `"planned"` is written after `--decompose`; pipelines later move to
 * `"running"` | `"done"` | `"failed"`. Not enforced by a runtime checker.
 */
export const SEQ_DOC_STATES = Object.freeze(['planned', 'running', 'done', 'failed']);
 ...
 * @param {number} [options.maxUnits] - Reject when units.length exceeds this.
 * @param {number} [options.minUnits=2] - Reject when units.length is below this.
 *   Default 2 keeps `--seq` requiring ≥2 units; pass `minUnits: 1` for
 *   `--decompose` so a single well-formed unit is accepted. Empty `units[]`
 *   (length 0) is always invalid because 0 is below any positive minUnits.
 */
export function validateSeqDecomposition(decomposition, { maxUnits, minUnits = 2 } = {}) {
    const violations = [];
    ...
    if (units.length < minUnits) {
        violations.push(
            minUnits === 2
                ? 'fewer than two units; not decomposable'
                : `fewer than ${minUnits} units; not decomposable`,
        );
    }
