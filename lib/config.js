...
function stripOneTrailingSlash(value) {
...
function isGitSafeNamespace(value) {
...
function assertGitSafeBranchPrefix(raw, displayPath) {
    const normalized = stripOneTrailingSlash(raw);
    if (!isGitSafeNamespace(normalized)) {
        const detail = `${JSON.stringify(raw)} (expected a git-safe namespace)`;
        throw new Error(
            displayPath ? `invalid branchPrefix in ${displayPath}: ${detail}` : `invalid branchPrefix: ${detail}`,
        );
    }
    return normalized;
}
 ...
 * invalid `agent` / `notify` / `branchPrefix` → throws with a message suitable for `Error: …`
...
export function loadConfig(configPath, displayPath = configPath) {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'branchPrefix') && data.branchPrefix !== undefined) {
        out.branchPrefix = assertGitSafeBranchPrefix(data.branchPrefix, displayPath);
    }
...
/**
 * Effective branch prefix for a run: local > global > `orch`.
 * Reads local then global; throws on bad files. No CLI override.
 */
export function resolveBranchPrefix({ cwd, homedir = os.homedir() } = {}) {
    const local = loadConfig(localConfigPath(cwd), '.orch/config');
    const global = loadConfig(globalConfigPath({ homedir }), '~/.orch/config');
    if (typeof local.branchPrefix === 'string') return local.branchPrefix;
    if (typeof global.branchPrefix === 'string') return global.branchPrefix;
    return 'orch';
}

/**
 * Create parent dirs if needed, merge `agent` / `notify` / `branchPrefix` into
 * the existing config object (preserve the other keys), overwrite with
 * pretty-printed JSON, return the path written.
 */
export function writeConfig(configPath, { agent, notify, branchPrefix } = {}) {
    ...
    if (branchPrefix !== undefined) {
        branchPrefix = assertGitSafeBranchPrefix(branchPrefix);
    }
    ...
    if (branchPrefix !== undefined) next.branchPrefix = branchPrefix;
    ...
    // Keep agent/notify/branchPrefix; preserve other keys already present.)
    const out = {};
    if (next.agent !== undefined) out.agent = next.agent;
    if (next.notify !== undefined) out.notify = next.notify;
    if (next.branchPrefix !== undefined) out.branchPrefix = next.branchPrefix;
    for (const [k, v] of Object.entries(next)) {
        if (k === 'agent' || k === 'notify' || k === 'branchPrefix') continue;
        out[k] = v;
    }
...
/**
 * Print effective agent/notify/branchPrefix and which file(s) contributed (stdout). Throws
...
export function printConfig({
    ...
    const branchPrefix = resolveBranchPrefix({ cwd, homedir });
    let branchPrefixSource = 'default (builtin)';
    if (typeof local.branchPrefix === 'string') {
        branchPrefixSource = `local (${localPath})`;
    } else if (typeof global.branchPrefix === 'string') {
        branchPrefixSource = `global (${globalPath})`;
    }

    log(`branchPrefix=${branchPrefix}`);
    log(`branchPrefixSource=${branchPrefixSource}`);
    log(`branchPrefixGlobal=${typeof global.branchPrefix === 'string' ? global.branchPrefix : 'unset'} (${globalPath})`);
    log(`branchPrefixLocal=${typeof local.branchPrefix === 'string' ? local.branchPrefix : 'unset'} (${localPath})`);
}
