 ...
 * assert without tearing down the runner. When `ORCH_JOB_SLUG` is set (a
 * detached child), synchronously persists terminal job state to `run.json`
...
export function shutdown(signal, { exit = (code) => process.exit(code), jobCwd = process.cwd() } = {}) {
    ...
    const jobSlug = process.env.ORCH_JOB_SLUG;
    if (jobSlug) {
        try {
