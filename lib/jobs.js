...
/**
 * Sibling worktree path used by `createWorktree` when repoRoot === cwd:
 * `<dirname(cwd)>/<basename(cwd)>-<slug>`.
 */
function createWorktreeSiblingPath(cwd, slug) {
    const resolved = path.resolve(cwd);
    return path.join(path.dirname(resolved), `${path.basename(resolved)}-${slug}`);
}

/**
 * Pure op behind `orch jobs delete <slug>`: leaf-only delete of one job
 * under `.orch/<slug>/`, force-removing an associated worktree when present
 * (recorded path or on-disk createWorktree sibling). Returns
 * `{ status: 'missing'|'blocked'|'deleted', slug, worktreeRemoved? }`
 * without throwing for those three outcomes.
 */
export function deleteJob(cwd, slug, opts = {}) {
    const record = readJob(cwd, slug);
    if (!record) {
    ...
    if (live.includes(slug)) {
        return { status: 'blocked', slug };
    }

    const removeWorktreeFn = opts.removeWorktree ?? defaultRemoveWorktree;
    const repoRoot = path.resolve(cwd);
    const branch = record.branch ?? `orch/${slug}`;

    let worktreePath = null;
    if (typeof record.worktree === 'string' && record.worktree.length > 0) {
        worktreePath = record.worktree;
    } else {
        const sibling = createWorktreeSiblingPath(cwd, slug);
    ...
    if (worktreePath) {
        ...
        if (opts.execFile !== undefined) {
            removeArgs.execFile = opts.execFile;
        }
        removeWorktreeFn(removeArgs);
        worktreeRemoved = true;
    }

    const { dir } = jobPaths(cwd, slug);
    fs.rmSync(dir, { recursive: true, force: true });

    return { status: 'deleted', slug, worktreeRemoved };
}
