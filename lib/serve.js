...
import { readAskSession } from './ask-session.js';
...
/**
 * Normalize seq.json for job API payloads (`job.seq`), or null when missing.
 * Propagates readSeq throws on corrupt JSON so GET handlers surface 500.
 */
function seqEnrichment(cwd, slug) {
    const doc = readSeq(cwd, slug);
    if (!doc) return null;
    const units = Array.isArray(doc.units) ? doc.units : [];
    return {
        state: doc.state,
        units: units.map((unit) => {
            const childSlug = unit.slug ?? null;
            return {
                id: unit.id,
                title: unit.title,
...
/** Attach `job.seq` when seq.json exists; leave the job unchanged when missing. */
function withSeqEnrichment(job, cwd) {
    const seq = seqEnrichment(cwd, job.slug);
    if (!seq) return job;
    return { ...job, seq };
}
...
function scanAllJobs(productsDir) {
    ...
    for (const { slug: product, cwd } of listProductDirs(productsDir)) {
        for (const job of listJobs(cwd)) {
            const base = { ...job, product: job.product ?? product };
            jobs.push(withSeqEnrichment(base, cwd));
        }
    }
    jobs.sort((a, b) => new Date(b.startedAt).getTime() - new Date(a.startedAt).getTime());
    return jobs;
...
function findJob(productsDir, slug) {
    for (const { slug: product, cwd } of listProductDirs(productsDir)) {
        ...
        if (job) {
            const base = { ...job, product: job.product ?? product };
            return { job: withSeqEnrichment(base, cwd), cwd, product };
        }
    }
    return null;
}
...
export async function startServe(options = {}) {
    ...
    /** Injectable non-exiting ask runner (CLI wires lib/ask-run.js; tests inject mocks). */
    let runAsk = typeof options.runAsk === 'function' ? options.runAsk : null;
    async function resolveRunAsk() {
        if (typeof runAsk === 'function') return runAsk;
        const mod = await import('./ask-run.js');
        runAsk = mod.runAsk;
        return runAsk;
    }
    ...
    async function startOne(job) {
        ...
        try {
            ...
            else if (job.mode === 'decompose') detachOpts.decompose = true;
    ...
    async function handle(req, res) {
        ...
        try {
            ...
            {
                const m = pathname.match(/^\/api\/jobs\/([^/]+)\/(pause|resume|stop|start)$/);
                if (method === 'POST' && m) {
                    const slug = decodeURIComponent(m[1]);
                    const action = m[2];
                    const found = findJob(productsDir, slug);
                    ...
                    // start: plan-only decompose → orch --seq --from <slug>
                    if (LIVE_STATES.has(found.job.state)) {
                        sendJson(res, 409, {
                            error: `job is live (${found.job.state}); cannot start`,
                        });
                        return;
                    }
                    if (found.job.seq?.state !== 'planned') {
                        sendJson(res, 409, {
                            error: 'job seq state must be planned to start',
                        });
                        return;
                    }
                    const prompt = found.job.task ?? found.job.seq?.task;
                    await runDetached(prompt, {
                        agent: found.job.agent,
                        maxRounds: found.job.maxRounds,
                        cwd: found.cwd,
                        pr: true,
                        base: found.job.base ?? base,
                        fromSlug: slug,
                        exit: () => {},
                    });
                    const refreshed = findJob(productsDir, slug);
                    sendJson(res, 200, { ok: true, job: refreshed?.job ?? found.job });
                    return;
                }
            }
            ...
            // Per-product ask API (read-only; never write-queue / runDetached).
            {
                const m = pathname.match(/^\/api\/products\/([^/]+)\/ask$/);
                if (method === 'POST' && m) {
                    const product = decodeURIComponent(m[1]);
                    const productCwd = path.join(productsDir, product);
                    ...
                    try {
                        const askFn = await resolveRunAsk();
                        ...
                        sendJson(res, status, { error: err.message || 'ask failed' });
                    }
                    return;
                }
            }

            {
                ...
                if (m) {
                    ...
                    if (method === 'GET') {
                        let session;
                        try {
                            session = readAskSession(productCwd, slug);
                        } catch (err) {
                            sendJson(res, 400, {
                                error: `could not read ask session for ${slug}: ${err.message}`,
                            });
                            return;
                        }
                        if (!session) {
                            sendJson(res, 404, { error: 'ask session not found' });
                            return;
                        }
                        const job = readJob(productCwd, slug);
                        sendJson(res, 200, { slug, session, job: job ?? null });
                        return;
                    }
...

            {
                ...
                if (method === 'POST' && m) {
                    ...
                    const hasDecomposeBool = body.decompose === true;
                    const exclusiveBoolCount =
                        (hasSeqBool ? 1 : 0) + (hasFanOutBool ? 1 : 0) + (hasDecomposeBool ? 1 : 0);
                    if (exclusiveBoolCount > 1) {
                        sendJson(res, 400, { error: 'seq, fanOut, and decompose cannot be combined' });
                        return;
                    }

                    let mode;
                    if (Object.prototype.hasOwnProperty.call(body, 'mode') && body.mode != null && body.mode !== '') {
                        if (body.mode !== 'seq' && body.mode !== 'fan-out' && body.mode !== 'decompose') {
                            sendJson(res, 400, { error: 'mode must be seq, fan-out, or decompose' });
                            return;
                        }
                        mode = body.mode;
                    } else if (hasSeqBool) {
                        ...
                        mode = 'fan-out';
                    } else if (hasDecomposeBool) {
                        mode = 'decompose';
                    }

                    if (mode === 'seq' && (hasFanOutBool || hasDecomposeBool)) {
                        sendJson(res, 400, { error: 'seq, fanOut, and decompose cannot be combined' });
                        return;
                    }
                    if (mode === 'fan-out' && (hasSeqBool || hasDecomposeBool)) {
                        sendJson(res, 400, { error: 'seq, fanOut, and decompose cannot be combined' });
                        return;
                    }
                    if (mode === 'decompose' && (hasSeqBool || hasFanOutBool)) {
                        sendJson(res, 400, { error: 'seq, fanOut, and decompose cannot be combined' });
                        return;
                    }
