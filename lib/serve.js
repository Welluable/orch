...
/** Read-only git name-status from a job worktree. Empty when unavailable. */
function listJobFiles(worktree, execFile) {
    if (!worktree || typeof worktree !== 'string') return [];
    if (!fs.existsSync(worktree)) return [];
    try {
        const out = execFile('git', ['-C', worktree, 'diff', '--name-status', 'HEAD'], {
            encoding: 'utf8',
        });
        return String(out ?? '')
            .split('\n')
...
/**
 * Start the orch serve HTTP process (jobs queue + products API).
 *
 * @returns {Promise<{ server, port, host, requestedHost, productsDir, close }>}
 */
