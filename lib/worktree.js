...
/**
 * Force-removes a job worktree and best-effort deletes its local branch.
 * When `worktreePath` exists on disk, runs `git worktree remove --force`.
 * When `branch` is a non-empty string, runs `git branch -D` afterward
 * (failures swallowed — already gone / checked out elsewhere).
 * `execFile` is injectable like `createWorktree`.
 */
export function removeWorktree({
    ...
    if (fs.existsSync(worktreePath)) {
        runGit(execFile, ['-C', repoRoot, 'worktree', 'remove', '--force', worktreePath]);
        removed = true;
    }

    if (typeof branch === 'string' && branch.length > 0) {
        try {
            execFile('git', ['-C', repoRoot, 'branch', '-D', branch]);
        } catch {
            // Best-effort: branch may already be gone or checked out elsewhere.
        }
    }

    return { worktreePath, branch, removed };
}
