 ...
 *
 * Continue accepts `done` jobs (follow-up work) as well as `failed`/
 * `stopped`/`crashed` complex runs with an existing worktree, for
 * exhausted-loop recovery: a fresh attempt from research with a new task
 * prompt, on the same slug/worktree, rather than resume's stage-cursor
 * reentry. Coordinators, integration jobs, and live states are still refused.
 */
